...
class PanelController extends Controller
{
    ...
    public function upload(Request $request, $folder, $waterMark = 0, $height = null, $width = null)
    {
        if ($request->hasFile('file')) {
            ...
            $photo = file_store($request->file, 'source/assets/uploads/' . $folder . '/' . my_jdate(date('Y/m/d'), 'Y-m-d') . '/', 'photo-');
            ...
            if ($waterMark == 1) {
                $img->insert('source/assets/user/pic/logo.png', 'bottom', 30, 0); //insert watermark in (also from public_directory)
            }

            $img->save($photo);
            //save created image (will override old image)
