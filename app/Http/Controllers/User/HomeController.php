...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $article = Article::where('status',1)->where('type','!=','radio_vandidad')->take(8)->get();
        $radios = Article::where('status',1)->where('type','radio_vandidad')->take(8)->get();
        $slider_mobile = Slider::where('type',1)->get();
        $slider_desktop = Slider::where('type',2)->get();
        $villas = Villa::where('status', 'published');
        $villasAsItems = $villas->orderBy('id', 'desc')->take('10')->get();
//        $cities = City::where('state_id',null)->where('city_id',null)->get();
        $states = City::where('state_id',null)->where('id',35)->get();
        $states_c = City::where('state_id',35)->where('city_id',null)->get();
        $cities = City::where('state_id',null)->get();
        $cats = Cat::where('hide',0)->OrderBy('title')->get();
        $cat_locales = CatLocale::OrderBy('title')->get();

        $minimumPrices = $villas->orderBy('price', 'asc')->pluck('price')->toArray();
        $numberOfRooms = $villas->pluck('room_num')->toArray();

        $locations = Location::all();
        $selectedLocations = CatLocale::where('status_home','active')->get();
        $villaCategories = Category::where('type', 'villa')->where('status', 'published')->orderBy('id','DESC')->take(4)->get();
...
//        $selectedLocations = Location::where('type',1)->orderBy('id', 'desc')->get();

        $galleries = Photo::where('type','gallery_Istanbul')->get();
        $teams=team::take(4)->get();
        $CustomerSliders=CustomerSlider::all();
        $boutSlider = boutSlider::findOrFail(1);
        $aboutSliders = aboutSlider::all();
        $abouts = about::all();
        $doviz = file_get_contents("https://www.doviz.com/");
        return view('user.index', compact('doviz','abouts','aboutSliders','boutSlider','CustomerSliders','teams','galleries','sliders','selectedLocations','cats','locations','states','states_c','cities','selectedLocations','price_max','price_min', 'villaCategories', 'numberOfRooms','price_max_all','price_min_all','area_min_all','area_max_all'
            , 'minimumPrices', 'villaTypes', 'priceRanges', 'villasAsItems','sleeps', 'projectTypes', 'typeVillas','article','slider_mobile','slider_desktop','response','price_select','area_select','radios','cat_locales'));
    }
...
//        return view('about', compact('about', 'slider', 'setting'));
    ...
    {
        $project = Category::where('id', $id)->first();
        $order_code = 0;
        ...
        if (!$project) {
            ...
            return ['status'=>'not','compare_count'=>$compare_count];
        }
        try {
            ...
            if (!session()->has('compare')) {
                $compares = [];
                array_push($compares, ['project_id' => $id, 'order_code' => (int)$order_code]);
                session(['order_code' => $order_code]);
                session(['compare' => $compares]);
            } else {
                $exist = false;
                ...
                if (!$exist) {
                    array_push($compare, ['project_id' => $id, 'order_code' => (int)$order_code]);
                    session(['compare' => $compare]);

                    $compare_end = session('compare');
                    $compare_count = count($compare_end);
                    return ['status'=>'add_ok','compare_count'=>$compare_count];
                } else {
                    try {
                        if (!session()->has('order_code')) {
                            $order_code = time();
                        ...
                        if (session()->has('compare')) {
                            $compare_del = session('compare');
                            $compare_dels=[];
                            $remove = false;
                            foreach ($compare_del as $key => $value) {
                                if ($value['project_id'] == $id) {
                                    if (count($compare_del) <= 1) {
                                        $compare_del = [];
                                    } else {
                                        unset($compare_del[$key]);
                                    }
                                    $remove = true;
                                }
                                else
                                {
                                    array_push($compare_dels, ['project_id' => $value['project_id'], 'order_code' => (int)$order_code]);
                                }
                            }
                            if ($remove) {
                                session(['compare' => $compare_dels]);

                                $compare_end = session('compare');
                                $compare_count = count($compare_end);
                                return ['status'=>'del_ok','compare_count'=>$compare_count];
                            } else {
                                $compare_end = session('compare');
                                $compare_count = count($compare_end);
                                return ['status'=>'not','compare_count'=>$compare_count];
    ...
    {
        $compare_end = session('compare');
        return view('user.compare.index',compact('compare_end'));
    }
    public function clear_compare()
    {
        ...
        $compare_end = session('compare');
        $compare_count = count($compare_end);
        return ['status'=>'ok_clear','compare_count'=>$compare_count];
    }
