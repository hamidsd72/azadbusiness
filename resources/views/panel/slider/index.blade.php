            ...
            <div class="card-header archive-card-header">
                <div class="archive-circle-wrap">
                    <div class="archive-circle">
                        <a href="{{url('/')}}" target="_blank"><img src="{{asset('images/logo-light.png')}}" style="margin-top: 10px;"></a>                    </div>
                    <h2>لیست {{ $title }}</h2>
...
{{--                                <td>@if($slider->type == 1) موبایل @elseif($slider->type == 2) دکستاپ @endif </td>--}}
                                <td><a href="{{ url($slider->photo->path) }}" target="_blank"><img src="{{ url($slider->photo->path) }}" height="150px"/></a></td>
                                <td width="140">
                                    @if(!auth()->user()->hasRole('watcher'))
                                    <div class="btn-inline">
                                        <a href="{{ route('slider-edit', $slider->id) }}" class="btn btn-sm btn-info float-left mx-1"><i class="fa fa-edit"></i></a>
                                        {!! Form::open(['method' => 'DELETE', 'route' => ['slider-destroy', $slider->id] ]) !!}
                                        {!! Form::button('<i class="fa fa-ban mx-1"></i>', ['type' => 'submit', 'class' => 'btn btn-danger float-left', 'onclick' => 'return confirm("آیا مطمئن هستید؟")']) !!}
                                        {!! Form::close() !!}
                                    </div>
                                    @endif
                                </td>
